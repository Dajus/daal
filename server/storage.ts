...
export interface IStorage {
  ...
  getStudentSessionByNameEmailAndCode(
    name: string,
    email: string,
    accessCodeId: number,
  ...
  getCompanyAccessCodesDetailed(companyId: number): Promise<
    Array<{
      id: number
      code: string
      courseName: string
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteCourse(id: number): Promise<void> {
    ...
    const courseAccessCodes = await db
    ...
    if (accessCodeIds.length > 0) {
      ...
      if (sessionIds.length > 0) {
        ...
        await db.delete(certificates).where(inArray(certificates.studentSessionId, sessionIds))

        // Then delete test attempts
        await db.delete(testAttempts).where(inArray(testAttempts.studentSessionId, sessionIds))
      }

      // Delete student sessions
      await db.delete(studentSessions).where(inArray(studentSessions.accessCodeId, accessCodeIds))
  ...
  async updateCompanyAdminLastLogin(id: number): Promise<void> {
    await db.update(companyAdmins).set({ lastLogin: new Date() }).where(eq(companyAdmins.id, id))
  }

  async deleteCompanyAdmin(id: number): Promise<void> {
    await db.update(companyAdmins).set({ isActive: false, updatedAt: new Date() }).where(eq(companyAdmins.id, id))
  }
  ...
  async updateCompanyAdmin(id: number, companyAdmin: InsertCompanyAdmin): Promise<CompanyAdmin> {
    ...
    const [updatedCompanyAdmin] = await db
  ...
  async getAllAccessCodes(): Promise<AccessCode[]> {
    return db.select().from(accessCodes).where(eq(accessCodes.isActive, true)).orderBy(desc(accessCodes.createdAt))
  }
  ...
  async deleteTheorySlide(id: number): Promise<void> {
    await db.update(theorySlides).set({ isActive: false, updatedAt: new Date() }).where(eq(theorySlides.id, id))
  }
  ...
  async getStudentSessionByEmailAndCode(email: string, accessCodeId: number): Promise<StudentSession | undefined> {
    const [session] = await db
      .select()
      .from(studentSessions)
      .where(and(eq(studentSessions.studentEmail, email), eq(studentSessions.accessCodeId, accessCodeId)))
    return session || undefined
  }

  async getStudentSessionByNameEmailAndCode(
    name: string,
    email: string,
    accessCodeId: number,
  ): Promise<StudentSession | undefined> {
  ...
  async markTheoryComplete(sessionId: number): Promise<{ theoryToTest: boolean }> {
    ...
    const [sessionWithCode] = await db
      .select({
    ...
    await db.update(studentSessions).set({ theoryCompletedAt: new Date() }).where(eq(studentSessions.id, sessionId))
  ...
  async getCertificateBySession(sessionId: number): Promise<Certificate | undefined> {
    const [certificate] = await db
      .select()
      .from(certificates)
      .where(and(eq(certificates.studentSessionId, sessionId), eq(certificates.isValid, true)))
  ...
  async getCertificateByVerificationCode(verificationCode: string): Promise<Certificate | undefined> {
    const [certificate] = await db
      .select()
      .from(certificates)
      .where(and(eq(certificates.verificationCode, verificationCode), eq(certificates.isValid, true)))
    return certificate || undefined
  }
  ...
  async getAnalytics() {
    ...
    const [activeCodesResult] = await db
      .select({ count: count() })
      .from(accessCodes)
      .where(and(eq(accessCodes.isActive, true), gte(accessCodes.validUntil, new Date().toISOString().split('T')[0])))
    ...
    const [inProgressResult] = await db.select({ count: count() }).from(studentSessions)
  ...
  async getAnalyticsByCompany(companyId: number) {
    ...
    const [completionsResult] = await db
      ...
      .innerJoin(studentSessions, eq(certificates.studentSessionId, studentSessions.id))
      .innerJoin(accessCodes, eq(studentSessions.accessCodeId, accessCodes.id))
      .where(and(eq(accessCodes.companyId, companyId), eq(certificates.isValid, true)))
    ...
    const [employeesTrainedResult] = await db
      .select({
    ...
    const [passedAttempts] = await db
      ...
      .innerJoin(studentSessions, eq(testAttempts.studentSessionId, studentSessions.id))
      .innerJoin(accessCodes, eq(studentSessions.accessCodeId, accessCodes.id))
      .where(and(eq(accessCodes.companyId, companyId), eq(testAttempts.passed, true)))
    ...
    const coursesOfferedData = await db
      ...
      .leftJoin(
        certificates,
        and(eq(studentSessions.id, certificates.studentSessionId), eq(certificates.isValid, true)),
      )
      .where(eq(accessCodes.companyId, companyId))
      .groupBy(courses.id, courses.name)
      .orderBy(desc(count(certificates.id)))
    ...
    const detailedCodes = await Promise.all(
      accessCodesData.map(async (accessCode) => {
        ...
        const studentsData = await Promise.all(
          basicStudentsData.map(async (student) => {
            ...
            const [certificate] = await db
              ...
              .from(certificates)
              .where(and(eq(certificates.studentSessionId, student.sessionId), eq(certificates.isValid, true)))
              .limit(1)
        ...
        const students = studentsData.map((student) => ({
          ...
          lastActivity:
            student.certificateIssuedAt || student.testCompletedAt || student.theoryCompletedAt || student.createdAt,
        }))
