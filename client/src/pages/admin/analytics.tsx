...
import { Key, GraduationCap, Clock, TrendingUp, Users, CheckCircle, XCircle, AlertCircle, ChevronDown } from "lucide-react";
                ...
                {(analytics.companyPerformance || []).map((company, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-sm text-gray-600 dark:text-gray-400">{company.employeesTrained} proškolených zaměstnanců</div>
                      </div>
                      <div className={`text-sm font-semibold ${
                          company.successRate >= 95 ? 'text-green-600 dark:text-green-400' :
                              company.successRate >= 90 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'
                      }`}>
                        {company.successRate}%
                      </div>
                    </div>
                  ...
                  {detailedLoading ? (
                      ...
                      <div className="space-y-4">
                        {(() => {
                          ...
                          const groupedCodes = (detailedCodes || []).reduce((groups, accessCode) => {
                                          ...
                                          <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                              ...
                                              <div className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">
                                                {codes.length} přístupových kódů • {codes.reduce((sum, code) => sum + Number(code.usage), 0)} celkových účastníků
                                              </div>
                                            </div>
                                          </div>
                                      ...
                                      <AccordionContent className="px-3 sm:px-4 pb-3 sm:pb-4 pt-2">
                                        <div className="space-y-4">
                                          {codes.map((accessCode, index) => (
                                              <Accordion key={index} type="multiple" className="border border-gray-200 dark:border-gray-600 rounded-lg">
                                                <AccordionItem
                                                    value={`access-code-${accessCode.id}`}
                                                  ...
                                                  <AccordionTrigger className="hover:no-underline pl-3 sm:pl-6 pr-2 sm:pr-3 py-2 bg-gray-50 dark:bg-gray-700/50 rounded-t-lg">
                                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full text-left gap-2">
                                                      <div className="flex items-center gap-3">
                                                        ...
                                                        <Badge variant={accessCode.isActive ? "default" : "secondary"} className="text-xs py-0">
                                                      ...
                                                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs text-gray-600 dark:text-gray-400">
                                                        <span>Využití: {accessCode.usage}{accessCode.maxParticipants && ` / ${accessCode.maxParticipants}`}</span>
                                                        <span>Platný do: {new Date(accessCode.validUntil).toLocaleDateString('cs-CZ')}</span>
                                                        {accessCode.theoryToTest && (
                                                            <span className="text-blue-600 dark:text-blue-400">✓ Automatický přechod</span>
                                                        )}
                                                      </div>
                                                    </div>
                                                  </AccordionTrigger>
                                                        ...
                                                        <div className="space-y-2">
                                                          ...
                                                          <div className="grid gap-2">
                                                            {accessCode.students.map((student, studentIndex) => (
                                                                <div key={studentIndex} className="bg-gray-50 dark:bg-gray-700/50 rounded p-2">
                                                                            ...
                                                                            <div className="relative group">
                                                                              ...
                                                                              {student.testCompletedAt && (
                                                                                  <div className="absolute -top-1 right-full mr-1 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                                                                                    {new Date(student.testCompletedAt).toLocaleDateString('cs-CZ', {
                                                                                      day: '2-digit',
                                                                                      month: '2-digit',
                                                                                      year: '2-digit'
                                                                                    })} {new Date(student.testCompletedAt).toLocaleTimeString('cs-CZ', {
                                                                                    hour: '2-digit',
                                                                                    minute: '2-digit'
                                                                                  })}
                                                                                  </div>
