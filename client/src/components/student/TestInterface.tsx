...
const TestInterface = ({ progress }: { progress?: any }) => {
        ...
        <CardContent className="p-8 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 max-w-4xl mx-auto">
            ...
            <div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div
                className={`text-3xl font-bold mb-2 ${testResult.attempt.passed ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
              >
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {testResult.questionsCount || questions.length} celkem
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Nutné skóre:</span>
                    <span className="font-medium text-gray-900 dark:text-white">
                      {progress?.course?.passingScore || 70}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Pokus číslo:</span>
                    <span className="font-medium text-gray-900 dark:text-white">
                      #{testResult.attempt.attemptNumber}
                    </span>
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {Math.round(
                        testResult.attempt.timeTakenSeconds / (testResult.questionsCount || questions.length),
                      )}
                      s průměr
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Hodnocení:</span>
                    <span
                      className={`font-medium ${testResult.attempt.passed ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                    >
                      {testResult.attempt.passed ? 'ÚSPĚCH' : 'NEÚSPĚCH'}
                    </span>
                  </div>
        ...
        <CardContent className="p-8">
          <div className="space-y-6">
                ...
                <div className="mt-2 space-y-1">
                  {progress.attempts.map((attempt: any, index: number) => (
                    <div key={attempt.id} className="flex justify-between text-sm text-yellow-800 dark:text-yellow-300">
                      ...
                      <span>
                        {attempt.percentage}% - {attempt.passed ? 'Úspěch' : 'Neúspěch'}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="text-center">
              <Button onClick={startTest} size="lg" className="bg-primary hover:bg-primary-dark px-8 py-3">
                Zahájit test
              </Button>
            </div>
          </div>
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={isAnswered ? 'default' : 'destructive'}
                            className={
                              isAnswered
                                ? 'bg-emerald-100 dark:bg-emerald-800/40 text-emerald-800 dark:text-emerald-300'
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{question.questionText}</p>
                    </div>
                  )
                })}
              </div>
          ...
          <CardContent className="p-3 sm:p-4 lg:p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-3 sm:p-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700">
              ...
              <div className="flex items-center gap-2 sm:gap-4 w-full sm:flex-1">
                ...
                {timeRemaining !== null && (
                  <div
                    className={`text-xs sm:text-sm font-semibold px-2 sm:px-3 py-1 rounded-full border ${
                      timeRemaining < 300
          ...
          <div className="flex justify-between items-start">
            ...
            >
              ...
              <span className="hidden sm:inline">
                {flaggedQuestions.has(currentQuestion.id) ? 'Označeno' : 'Označit'}
              </span>
