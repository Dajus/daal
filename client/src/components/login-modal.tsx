...
const LoginModal = ({ open, onOpenChange, type = 'select' }: LoginModalProps) => {
  const [mode, setMode] = useState<'select' | 'admin' | 'student'>(type === 'student' || type === 'admin' ? type : 'select');
  ...
  // Resetovat režim při změně type prop
  useEffect(() => {
    if (open) {
      const newMode = type === 'student' || type === 'admin' ? type : 'select';
      setMode(newMode);
              ...
              <form onSubmit={handleStudentSubmit} className="space-y-4">
                ...
                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    <strong>Sdílené zařízení:</strong> Pokud používáte sdílené počítače/tablety, váš pokrok se uloží k vašemu jménu a e-mailu.
                  </p>
                </div>
